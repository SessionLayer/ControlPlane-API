 ...
 * {@code NodeConnection}. Public material only (host-CA and pinned host public
 * keys, SSH wire-encoded) — never a private key, and never TOFU: an empty
 * verification set is a misconfigured node that the Gateway aborts (fail
 * closed, §9.3).
 */
public record NodeConnectionInfo(ConnectorModel connectorKind, String dialAddress, List<byte[]> hostCaKeys,
		List<String> expectedPrincipals, List<byte[]> pinnedHostKeys) {
