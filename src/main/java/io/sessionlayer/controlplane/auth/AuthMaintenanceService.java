...
/**
 * Reaps the transient authentication rows so they do not grow unbounded: expired
 * OIDC login state, old rate-limit windows, past-expiry device flows, and
 * past-lifetime consumed client-assertion jtis. Runs on startup and hourly. Only
 * stale rows are deleted (a short grace preserves rows a live poll / audit
...
public class AuthMaintenanceService {
	...
	private static final String[] PRUNES = {
			"DELETE FROM runtime.consumed_assertion WHERE not_after < now()",
	...
	private Mono<Void> prune(String trigger) {
		...
		return chain.doOnSuccess(v -> LOG.debug("auth maintenance prune ({}) complete", trigger))
				.onErrorResume(e -> {
