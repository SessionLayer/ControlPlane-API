...
/**
 * The internal mTLS CA signing seam (X.509). Mirrors the SSH cloud-backend seams
 * ({@code KmsCaBackend}/{@code AzureKeyVaultCaBackend}/{@code VaultSshEngine}): a
 * single injectable interface behind which the CA private key lives, so a cloud
 * X.509 backend (KMS/Key Vault CA) can be dropped in without changing the
 * enrollment/renewal/server-cert code.
 *
 * <p>
 * Only {@link LocalX509CaBackend} (KEK-wrapped local key) is implemented for real
 * this session; the cloud seam is unit-tested with a double. Whatever the
