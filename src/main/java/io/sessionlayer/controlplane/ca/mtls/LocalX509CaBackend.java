 ...
 * with an in-memory ECDSA CA private key that was decrypted transiently from its
 * KEK-wrapped form (mirrors {@code LocalCaBackend} for the SSH CAs). The private
 * key is a JCA {@link PrivateKey} that cannot itself be zeroized, so — like the
 * SSH local backend — the resident-key exposure is inherent to a local software
 * signer; production SHOULD use a cloud X.509 backend behind the
