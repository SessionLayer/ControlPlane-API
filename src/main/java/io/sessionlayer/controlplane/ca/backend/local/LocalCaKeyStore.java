...
public final class LocalCaKeyStore {
	 ...
	 * {@code kek}.
	 */
	public GeneratedKey generate(CaKeyType keyType, Kek kek) {
		KeyPair keyPair = generateKeyPair(keyType);
		byte[] pkcs8 = keyPair.getPrivate().getEncoded();
		try {
			Kek.Wrapped wrapped = kek.wrap(pkcs8);
	...
	public LocalCaBackend load(CaKeyType keyType, Kek kek, Kek.Wrapped wrapped, byte[] publicKeyX509) {
		byte[] pkcs8 = kek.unwrap(wrapped.iv(), wrapped.ciphertext());
		try {
